	...
	const Vector* pos, int entity, int index )
{
	...
	if ( r_decals.GetInt() )
	{
		effects->DecalShoot( index, entity, ent->GetModel(), ent->GetEngineObject()->GetAbsOrigin(), ent->GetEngineObject()->GetAbsAngles(), *pos, 0, FDECAL_PERMANENT );
	}
}
...
void C_TEBSPDecal::PostDataUpdate( DataUpdateType_t updateType )
{
	...
	if ( r_decals.GetInt() )
	{
		effects->DecalShoot( m_nIndex, m_nEntity, ent->GetModel(), ent->GetEngineObject()->GetAbsOrigin(), ent->GetEngineObject()->GetAbsAngles(), m_vecOrigin, 0, FDECAL_PERMANENT );
	}
}
