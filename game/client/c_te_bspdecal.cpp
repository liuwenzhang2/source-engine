...
{
	C_BaseEntity *ent;
	if ( ( ent = (C_BaseEntity*)EntityList()->GetEnt( entity ) ) == NULL )
	{
	...
	if ( r_decals.GetInt() )
	{
		effects->DecalShoot( index, entity, ent->GetEngineObject()->GetModel(), ent->GetEngineObject()->GetAbsOrigin(), ent->GetEngineObject()->GetAbsAngles(), *pos, 0, FDECAL_PERMANENT );
	}
}
...
void C_TEBSPDecal::PostDataUpdate( DataUpdateType_t updateType )
{
	...
	C_BaseEntity *ent;
	if ( ( ent = (C_BaseEntity*)EntityList()->GetEnt( m_nEntity ) ) == NULL )
	{
	...
	if ( r_decals.GetInt() )
	{
		effects->DecalShoot( m_nIndex, m_nEntity, ent->GetEngineObject()->GetModel(), ent->GetEngineObject()->GetAbsOrigin(), ent->GetEngineObject()->GetAbsAngles(), m_vecOrigin, 0, FDECAL_PERMANENT );
	}
}
