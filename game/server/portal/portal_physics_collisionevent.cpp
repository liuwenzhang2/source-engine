	...
	if( pGameData0 != pGameData1 )
	{
		...
		if( pSimulators[0] == pSimulators[1] ) //same simulator
		{
			if( pSimulators[0] != NULL ) //and not main world
			{
					...
					for( int i = 0; i != 2; ++i )
					{
						if( bStatic[i] )
						{
							if( CPortalSimulator::IsPortalSimulatorCollisionEntity( pEntities[i] ) )
							{
								...
								{
									if( (pSimulators[1-i]->EntFlags[pEntities[1-i]->entindex()] & PSEF_IS_IN_PORTAL_HOLE) == 0 )
				...
				{
					if( ((pSimulators[0]->EntFlags[pEntities[0]->entindex()] | 
						pSimulators[1]->EntFlags[pEntities[1]->entindex()]) &
			...
			else
			{
				...
				for( int i = 0; i != 2; ++i )
				{
					if( pSimulators[i] )
					{
						...
						if( (pSimulators[i]->EntFlags[pEntities[1-i]->entindex()] & PSEF_CLONES_ENTITY_FROM_MAIN) == 0 ) //not cloned from main
							return 0;
					}
				}
			}
