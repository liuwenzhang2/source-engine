...
void CAI_PlaneSolver::GenerateObstacleNpcs( const AILocalMoveGoal_t &goal, float probeDist )
{
	if ( !ProbeForNpcs() )
	{
		...
		m_pNpc->GetEngineObject()->WorldSpaceSurroundingBounds( &minsSelf, &maxsSelf );
		...
		for ( int i = 0; i < g_AI_Manager.NumAIs(); i++ )
		{
			...
			if ( pAI != m_pNpc && pAI->IsAlive() && ( !goal.pPath || pAI != goal.pPath->GetTarget() ) )
			{
				...
				pAI->GetEngineObject()->WorldSpaceSurroundingBounds( &mins, &maxs );
		...
		if ( pPlayer )
		{
			...
			pPlayer->GetEngineObject()->WorldSpaceSurroundingBounds( &mins, &maxs );
			if ( mins.z < maxsSelf.z + 12.0 && maxs.z > minsSelf.z - 12.0 )
			{
...
bool CAI_PlaneSolver::GenerateCircleObstacleSuggestions( const AILocalMoveGoal_t &moveGoal, float probeDist )
{
	...
	m_pNpc->GetEngineObject()->WorldSpaceSurroundingBounds( &mins, &maxs );
	...
	for ( int i = 0; i < m_Obstacles.Count(); i++ )
	{
		...
		if ( m_Obstacles[i].hEntity && m_Obstacles[i].hEntity->GetEngineObject()->GetCollideable())
		{
			...
			pObstacleEntity->GetEngineObject()->WorldSpaceSurroundingBounds( &mins, &maxs );
