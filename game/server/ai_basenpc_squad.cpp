...
void CAI_BaseNPC::CheckSquad()
{
	...
	if(GetEngineObject()->VPhysicsGetObject() != NULL && (GetEngineObject()->VPhysicsGetObject()->GetGameFlags() & FVPHYSICS_PLAYER_HELD) )
	{
	...
	while ( pSquadmate )
	{
		if( IRelationType(pSquadmate) < D_LI )
		{
			...
			if( pSquadmate->GetEngineObject()->VPhysicsGetObject() != NULL )
			{
				if (pSquadmate->GetEngineObject()->VPhysicsGetObject()->GetGameFlags() & FVPHYSICS_PLAYER_HELD)
				{
					bWarn = false;
				}
			}	
...
int CAI_BaseNPC::NumWeaponsInSquad( const char *pszWeaponClassname )
{
	...
	if( !GetSquad() )
	{
		if( GetActiveWeapon() && GetActiveWeapon()->GetEngineObject()->GetClassname() == iszWeaponClassname)
		{
	...
	while ( pSquadmate )
	{
		if( pSquadmate->GetActiveWeapon() && pSquadmate->GetActiveWeapon()->GetEngineObject()->GetClassname() == iszWeaponClassname)
		{
			count++;
		}
		pSquadmate = m_pSquad->GetNextMember( &iter );
