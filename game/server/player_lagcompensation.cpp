...
void CLagCompensationManager::FrameUpdatePostEntityThink()
{
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		record.m_vecMaxsPreScaled	= pPlayer->GetEngineObject()->OBBMaxsPreScaled();
...
void CLagCompensationManager::BacktrackPlayer( CBasePlayer *pPlayer, float flTargetTime )
{
	...
	if ( minsPreScaled != pPlayer->GetEngineObject()->OBBMinsPreScaled() || maxsPreScaled != pPlayer->GetEngineObject()->OBBMaxsPreScaled() )
	{
...
void CLagCompensationManager::FinishLagCompensation( CBasePlayer *player )
{
	...
	for ( int i = 1; i <= gpGlobals->maxClients; i++ )
	{
		...
		if ( restore->m_fFlags & LC_SIZE_CHANGED )
		{
			...
			//  leave new values in
			if ( pPlayer->GetEngineObject()->OBBMinsPreScaled() == change->m_vecMinsPreScaled &&
