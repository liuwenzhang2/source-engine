...
public:
	...
	virtual void Save( const SaveRestoreFieldInfo_t &fieldInfo, ISave *pSave ) 
	{
		...
		const char *pActivityName = mdlcache->ActivityList_NameForIndex( activityIndex );
		if ( !pActivityName )
		{
			AssertOnce( activityIndex == -1 ); // FIXME: whatever activity this was, it's now being saved out as ACT_RESET
			pActivityName = mdlcache->ActivityList_NameForIndex( 0 );
		}
	...
	virtual void Restore( const SaveRestoreFieldInfo_t &fieldInfo, IRestore *pRestore ) 
	{
		...
		*pActivityIndex = mdlcache->ActivityList_IndexForName( nameBuf );
		if ( *pActivityIndex < 0 )
		{
			*pActivityIndex = 0;
		}
...
void UTIL_LoadActivityRemapFile( const char *filename, const char *section, CUtlVector <CActivityRemap> &entries )
{
	...
	if ( pkvFile->LoadFromFile( filesystem, filename, NULL ) )
	{
		...
		while ( pTestKey )
		{
			Activity ActBase = (Activity)mdlcache->ActivityList_IndexForName( pTestKey->GetName() );
			...
			if ( ActBase != ACT_INVALID )
			{
				...
				while ( pRemapKey )
				{
					...
					{
						Activity Act = (Activity)mdlcache->ActivityList_IndexForName( pKeyValue );

						if ( Act == ACT_INVALID )
						{
							actMap.mappedActivity = (Activity)mdlcache->ActivityList_RegisterPrivateActivity( pKeyValue );
						}
						else
						{
