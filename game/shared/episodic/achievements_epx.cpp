...
protected:
	...
	void FireGameEvent_Internal( IGameEvent *event )
	{
		if ( 0 == Q_strcmp( event->GetName(), "physgun_pickup" ) )
		{
			...
			IServerEntity *pEntityPickedUp = EntityList()->GetBaseEntity( event->GetInt( "entindex" ) );
...
protected:
	...
	void FireGameEvent_Internal( IGameEvent *event )
	{
		...
		{
			IServerEntity *pEntityIgnited = EntityList()->GetBaseEntity( event->GetInt( "entindex" ) );
